...
<c:if test="${empty sessionScope.uid }"> <!-- 세션에 uid가 존재하지 않을 때 login표시가 나오게 만들기 -->
	<a href="../login/login.do">login</a>	
</c:if>
	|	
<c:if test="${not empty sessionScope.uid }"> <!-- 세션에 uid가 존재할 때는 logout표시가 나오게 만들기 -->
	<a href="../login/logoutProc.do">logout</a>	
</c:if>
<br />
<c:if test="${not empty uid }"> <!-- 세션에 uid가 존재할 때 인사글 나오게 만들기 -->
	${uid }님 반갑습니다.	
</c:if>
